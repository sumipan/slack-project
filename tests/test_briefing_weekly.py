...
class TestCalcPeriod:
    def test_current_on_wednesday(self):
        # Wednesday = weekday 2; days_since_sat = (2-5)%7 = 4
        # this_saturday = wed - 4 days = previous Saturday
        wednesday = datetime(2026, 5, 27, 12, 0, 0, tzinfo=_JST)  # Wed
    ...
    def test_current_on_saturday(self):
        saturday = datetime(2026, 5, 23, 10, 0, 0, tzinfo=_JST)  # Saturday
        with patch("slack_project.briefing.weekly.datetime") as mock_dt:
            mock_dt.now.return_value = saturday
            mock_dt.strptime = datetime.strptime
            start, end = _calc_period("current", None, None)

        assert start.date() == saturday.date()  # today = Saturday
        assert start.hour == 0
        assert end.weekday() == 4  # next Friday
        assert end.hour == 23
...
class TestFetchSlackLog:
    ...
    def test_http_429_error(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        proj_dir = tmp_path / "projects" / "my-project"
        proj_dir.mkdir(parents=True)
        (proj_dir / "config.yaml").write_text(
    ...
    def test_missing_project_config(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        proj_dir = tmp_path / "projects" / "nonexistent-project"
        proj_dir.mkdir(parents=True)
        # No config.yaml → no token/channel

        ok, msg = fetch_slack_log("nonexistent-project", week="last")
        assert ok is False

    def test_missing_token(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        proj_dir = tmp_path / "projects" / "no-token"
        proj_dir.mkdir(parents=True)
        (proj_dir / "config.yaml").write_text("slack:\n  channel_id: C123\n")

        ok, msg = fetch_slack_log("no-token")
        assert ok is False
        assert "トークン" in msg
...
class TestPostSummary:
    def test_dry_run(self, tmp_path, monkeypatch):
        ...
        ok, msg = post_summary("my-project", summary, dry_run=True)
        assert ok is True
    ...
    def test_success(self, tmp_path, monkeypatch):
        ...
        with patch("requests.post", return_value=mock_resp):
            ok, msg = post_summary("my-project", summary)

        assert ok is True

    def test_missing_summary_file(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        ok, msg = post_summary("my-project", tmp_path / "nonexistent.md")
        assert ok is False
        assert "見つかりません" in msg


class TestRunAutoUpdate:
    def test_dry_run_no_projects(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
        ok, msg = run_auto_update(dry_run=True)
        assert ok is True
        assert "見つかりません" in msg

    def test_dry_run_with_projects(self, tmp_path, monkeypatch):
        monkeypatch.chdir(tmp_path)
